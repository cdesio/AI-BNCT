...
G4double TsDNAKRIonIonisationScaledModel::CrossSectionPerVolume(const G4Material* material,
    ...
    if(waterDensity!= 0.0) {
        ...
        // This part is to calculate CS for 4 ionizationshells
        G4double* Buffer = new G4double[tableData->NumberOfComponents()];
        const size_t n(tableData->NumberOfComponents());
        size_t i(n);
        G4double value = 0.;

        // open my file 
        // print out current KE, CS for different shells, related weight.
        // FILE* CSFile;
        // CSFile=fopen("EdepDetail.txt","a+");
        // show error if don't open
        ...
        // fprintf(CSFile,"Kinetic energy: %e\n",k);

        while (i>0)
        {
            i--;
            Buffer[i] = tableData->GetComponent(i)->FindValue(k);

            value += Buffer[i];
            // fprintf(CSFile,"Component %zu: %e\n",
            // i,
            // Buffer[i]);
        }
    
        // fprintf(CSFile,"total weight: %e\n\n",
        // value);
        ...
        // print out energy, ionization shell number, related CS.
        // FILE* myFile;
        // myFile=fopen("CS.txt","a+");
        // fprintf(myFile,"%e %e %i\n",
        // k/eV,
        // sigma/cm/cm,
        // ionizationShell);
        // fclose(myFile);    
...
void TsDNAKRIonIonisationScaledModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
    ...
    if (k >= lowLim && k < highLim) {
        ...
        // G4cout << " deexSecEnergy is "
        //     << deexSecEnergy
        //     << "secNumberInit is " << secNumberInit
        //     << "secNumberFinal is " << secNumberFinal
        //     << G4endl;
        ...
        //check it
        if (!statCode) {
            // k is particle->GetKineticEnergy();
            // scatteredEnergy = k-bindingEnergy-secondaryKinetic;
            // bindingEnergy = waterStructure.IonisationEnergy(ionizationShell);
            // so scatteredEnergy is remaining energy of primary particle after one interaction?
            // for stationary and non stationary, the edep is different
        ...
        // FILE* myFile;
        // myFile=fopen("EdepDetail.txt","a+");
        // fprintf(myFile,"%e %e %e %e %e %e\n",
        // k/eV,
        // scatteredEnergy/eV,
        // secondaryKinetic/eV,
        // deexSecEnergy/eV,
        // bindingEnergy/eV,
        // k-scatteredEnergy-secondaryKinetic-deexSecEnergy);
        // fclose(myFile);                                                          
    }
...
{  
    ...
    // open my file -give up, doesn't work
    // FILE* myFile;
    // myFile=fopen("EdepDetail.txt","a+");
    ...
    // fprintf(myFile,"Kinetic energy: %e\n",k);
    ...
    while (i>0)
    {
        ...
        // fprintf(myFile,"Component %zu: %e\n",
        // i,
        // valuesBuffer[i]);
    }
    
    
    // fprintf(myFile,"total weight: %e\n\n",
    // value);
