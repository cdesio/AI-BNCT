...
inline Command* GetPhaseSpaceInputCommand(CommandLineParser* parser)
{
  if (!parser) return nullptr;
  if (auto* command = parser->GetCommandIfActive("-in")) return command;
  if (auto* command = parser->GetCommandIfActive("-PS")) return command;
  return parser->GetCommandIfActive("-decayPS");
}

inline G4bool IsPhaseSpaceInputActive(CommandLineParser* parser)
{
  return GetPhaseSpaceInputCommand(parser) != nullptr;
}

inline G4String GetCommandOptionOrDefault(CommandLineParser* parser, const G4String& marker)
{
  if (!parser) return "";
  if (auto* command = parser->GetCommandIfActive(marker))
  {
    if (command->GetOption() != "") return command->GetOption();
  }
  if (auto* command = parser->FindCommand(marker))
  {
    return command->GetDefaultOption();
  }
  return "";
}
}
